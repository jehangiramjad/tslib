...
class SVDModel(object):
    ...
    def denoisedDFNew(self,D, missingValueFill = True):
        if (missingValueFill == True):
            ...
            diff = 0.5*(min + max)
            D[np.isnan(D)] = diff
        self.updateSVD(self, D)
        NewColsDenoised = tsUtils.matrixFromSVD(self.sk, self.Uk, self.Vk[:,-D.shape[1]:], probability=self.p)

        return NewColsDenoised.flatten()
