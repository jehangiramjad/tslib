...
class SqlImplementationTSDB(Interface):
    ...
    def get_time_series(self, name, start, end, connection = None, value_column="ts", index_col='"rowID"', Desc=False):
    ...
    def get_U_row(self, table_name, tsrow_range, models_range,k, connection, return_modelno = False):
    ...
    def get_V_row(self, table_name, tscol_range,k,connection, models_range = [0,10**10], return_modelno = False):
        ...
        #     result = connection.execute(query, (tscol_range[0], tscol_range[1],)).fetchall()
        # else:
        query = "SELECT " + columns + " FROM " + table_name + " WHERE tscolumn >= %s and tscolumn <= %s ; "
        result = connection.execute(query, (tscol_range[0], tscol_range[1],)).fetchall()
        return np.array(result)

    def get_S_row(self, table_name, models_range, k, connection ,return_modelno = False):
        ...
        result = connection.execute(query, (models_range[0], models_range[1],)).fetchall()
        return np.array(result)

    def get_SUV(self, table_name, tscol_range, tsrow_range, models_range, k, connection ,return_modelno = False):
            ...
            # if return_modelno :
            #     columns = 'modelno, '+columns
            query = "SELECT "+ columns +" FROM " + table_name + "_s WHERE modelno = %s or modelno = %s;"
            S = connection.execute(query, (models_range[0], models_range[1],)).fetchall()
            
            columns = self.vcol
            query = "SELECT " + columns + " FROM " + table_name + "_v WHERE tscolumn = %s; "
            V = connection.execute(query, (tscol_range[0],)).fetchall()
            
            columns = self.ucol
            query = "SELECT "+ columns +" FROM " + table_name + "_u WHERE tsrow =  %s and (modelno = %s or modelno = %s); "
            U = connection.execute(query,
                                         (tsrow_range[0], models_range[0], models_range[1],)).fetchall()
            return U,S,V
            

    def get_coeff(self, table_name, column, connection):
