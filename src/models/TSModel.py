...
class TSmodel(object):
    ...
    def UpdateModel(self, NewEntries, patch = False):
        ...
        if len(self.models) == 1 and len(NewEntries) < self.T/2: UpdateChunk = int(np.sqrt(self.T0))
        else:    UpdateChunk = 50 * self.L
        ...
        i = -1
        print "Entries left to train: ", len(NewEntries)
        if len(NewEntries)/(self.T/2.0) > self.gamma:
            
            self.updateTS(NewEntries[:])
            self.fitModels()
            return 

        print "Entries left to train by Chunksize: ", len(NewEntries)
    ...
    def WriteModel(self, ModelName = 'model', host = "localhost",database="querytime_test", user="aalomar", password="AAmit32lids"):
        ...
        c6 = 'CREATE MATERIALIZED VIEW '+tableNames[3]+'_view AS SELECT coeffpos, avg(coeffvalue) as All,\
  avg(coeffvalue) FILTER (WHERE modelno <= %s and modelno > %s -10) as Last10,\
  avg(coeffvalue) FILTER (WHERE modelno <= %s and modelno > %s -20) as Last20,\
  avg(coeffvalue) FILTER (WHERE modelno <= %s and modelno > %s -100) as Last100\
 FROM '+tableNames[3]+ ' \
group by coeffpos'
        c7 = 'CREATE INDEX ON '+tableNames[0]+' (modelno);'
        cur.execute(c1)
        cur.execute(c2)
        cur.execute(c3)
        cur.execute(c4)
        cur.execute(c5)
        cur.execute(c6, (lasModel,lasModel,lasModel,lasModel,lasModel,lasModel))
        cur.execute(c7)
        conn.commit()
        cur.close()
        conn.close()
